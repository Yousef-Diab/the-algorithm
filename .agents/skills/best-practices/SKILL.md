...
# Update dependencies when possible, then re-run the audit
pnpm update
pnpm audit
npm update
npm audit
...
Use the package manager that matches the project lockfile: `pnpm-lock.yaml` -> pnpm, `package-lock.json` -> npm, `yarn.lock` -> yarn.
...
- [ ] No vulnerable dependencies (`pnpm audit`, `npm audit`, or `yarn audit`)
...
| Tool | Purpose |
|------|---------|
| `pnpm audit` / `npm audit` / `yarn audit` | Dependency vulnerabilities |
